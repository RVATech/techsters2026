...
**Students should read [START-HERE.md](START-HERE.md).** This file is for
whoever is running the session.

## How it works

The assistant's behaviour is set by four files:

| File | Role |
| --- | --- |
| `context.md` | The ground rules — tone, encouragement, and the no-technical-words rule. Read first, outranks everything. |
| `.bolt/prompt` | Always-on summary of those rules. |
| `.bolt/skills/*.md` | Four specialists, loaded when relevant. |
| `GAME-PLAN.md` | Fills in with the student's decisions as she makes them. |

### The four skills

| Skill | Handles |
| --- | --- |
| **game-designer** | The opening conversation. What the game is, who's in it, what you do, how you win and lose. Asks lots of questions, records answers, doesn't build anything yet. |
...
START-HERE.md         what a student reads
GAME-PLAN.md          fills in with her decisions
.bolt/
  prompt              always-on instructions
  skills/             the four specialists
...
The tone and rules are all in `context.md` and the four skill files, in plain
